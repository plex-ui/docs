...
- **Under 5 engineers:** Use an existing component library. [Plex UI](/docs/components), shadcn, Radix Themes — pick one that matches your stack and design taste, then commit to it. Customization at this stage means adjusting tokens, not forking components.
...
**Pick a component library.** Browse [Plex UI components](/docs/components) or another library that fits your stack. Install it. Use it for one real screen before debating customization.
...
If you are using a [Figma-to-code bridge](/bridge), connect it now. The cost of setting it up is an hour. The cost of not having it is every future handoff where the designer and the AI editor disagree on padding.
