...
Plex UI uses a [nine-step control scale](/blog/why-9-sizes) from 22px to 48px across Button, Input, Select, SelectControl, and SegmentedControl. This matters for AI-generated interfaces because the model can pick precise intent without custom CSS:
...
Plex UI exists to make that contract explicit: shared [components](/docs/components), a full control-size scale, and token architecture that maps cleanly from Figma to React with [AI Bridge](/bridge).
