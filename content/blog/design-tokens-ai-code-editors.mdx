...
<script type="application/ld+json" dangerouslySetInnerHTML={{__html: JSON.stringify({"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":"What are design tokens and why do AI code editors miss them?","acceptedAnswer":{"@type":"Answer","text":"Design tokens are named values (like spacing/lg or surface/primary) that store design intent. AI editors miss them because they operate on raw CSS values from training data, not the semantic token layer defined in your design system."}},{"@type":"Question","name":"How to make AI code editors use design tokens?","acceptedAnswer":{"@type":"Answer","text":"Give the AI direct access to your token map via tools like AI Bridge, which reads variable bindings from Figma. Include your token names in prompts or project instructions so the model knows which token to use for each context."}},{"@type":"Question","name":"Why does AI-generated CSS drift from the design system?","acceptedAnswer":{"@type":"Answer","text":"AI models write plausible values like padding: 16px that look correct in isolation but miss the design system's token scale. Without knowing that 16px means --spacing-xl, the code cannot adapt to breakpoints, density changes, or theme switches."}}]})}} />
