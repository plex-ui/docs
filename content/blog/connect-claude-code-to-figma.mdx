...
<script type="application/ld+json" dangerouslySetInnerHTML={{__html: JSON.stringify({"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":"Can Claude Code read Figma files directly?","acceptedAnswer":{"@type":"Answer","text":"Not by default. Claude Code needs AI Bridge, a local Figma plugin that exposes an HTTP API on localhost:8867, allowing Claude to read component structure, design tokens, and variable bindings from your Figma file."}},{"@type":"Question","name":"How do I connect Claude Code to my Figma designs?","acceptedAnswer":{"@type":"Answer","text":"Install the AI Bridge plugin in Figma, start the bridge server on your machine, and paste the Bridge API instructions into your Claude Code session. Claude can then query your Figma file via HTTP requests to localhost:8867."}},{"@type":"Question","name":"Does Claude Code support Figma design tokens?","acceptedAnswer":{"@type":"Answer","text":"Yes, when connected through AI Bridge. Claude reads variable bindings from Figma nodes and generates CSS with var() references to your design tokens instead of hardcoded pixel values."}}]})}} />
...
The most common workflow: Claude reads a Figma component and [generates matching React code](/blog/figma-to-react-with-ai).
...
This is the workflow that keeps Claude Code output aligned with design intent — not through prompt engineering or manual review, but through shared infrastructure. The same approach works whether you are using Claude, [Cursor](/blog/connect-cursor-to-figma), or [Codex](/blog/connect-codex-to-figma). The constraint is the token system, not the model.
