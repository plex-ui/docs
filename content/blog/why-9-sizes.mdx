...
The important part is not only the number of steps. It is that the same scale applies across [primary controls](/docs/components): Button, Input, Select, SelectControl, and SegmentedControl.
...
That is the core reason Plex UI standardizes on nine sizes across controls: fewer local overrides, better [Figma-to-code parity](/bridge), and a design language that holds under real-world pressure.
