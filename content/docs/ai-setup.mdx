...
Download a comprehensive reference file that gives your AI editor complete knowledge of all {COMPONENTS_COUNT} components, 14 hooks, design tokens, and usage patterns.
...
Codex auto-loads `AGENTS.md` from the git repo root on every session (up to 32 KiB). For a user-level install that applies to every project, write to `~/.codex/AGENTS.md` instead. See the [Codex AGENTS.md docs](https://developers.openai.com/codex/guides/agents-md).

For a deeper integration — slash-command skill, auto-invocation, UI metadata — use the [Plex UI skill](/docs/skills#codex) instead.
...
- **{COMPONENTS_COUNT} components** — import paths, props, types, defaults, sub-components
- **{ICONS_COUNT} icons** — import examples and naming conventions
...
- Icons: `import { IconName } from "@plexui/ui/components/Icon"` — {ICONS_COUNT} icons available
...
Prefer to install manually? Follow the [Installation guide](/docs/installation).

Want richer AI integration? Set up the [MCP Server](/docs/mcp) for interactive component discovery.
