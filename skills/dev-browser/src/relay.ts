    ...
    upgradeWebSocket(() => {
      return {
        onOpen(_event, ws) {
          ...
          // Keep-alive: ping every 15s to prevent Chrome MV3 service worker idle timeout
          const keepAliveInterval = setInterval(() => {
            if (extensionWs === ws) {
              try { ws.send(JSON.stringify({ method: "ping" })); } catch { clearInterval(keepAliveInterval); }
            } else {
              clearInterval(keepAliveInterval);
            }
          }, 15000);
          (ws as any).__keepAlive = keepAliveInterval;
        },
        ...
        onClose(_event, ws) {
          ...
          if ((ws as any).__keepAlive) clearInterval((ws as any).__keepAlive);
          for (const pending of extensionPendingRequests.values()) {
