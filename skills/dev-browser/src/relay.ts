...
export async function serveRelay(
		...
		heartbeatTimer = setInterval(() => {
			if (extensionWs) {
				try {
					...
					log("Heartbeat send failed — extension connection may be dead");
				}
			}
	...
	function cancelGracePeriod() {
		if (gracePeriodTimer) {
			...
			log("Grace period cancelled — extension reconnected");
		}
	}
	...
	function teardownState() {
		...
		log("State torn down after grace period expired");
	}
	...
	app.get(
		...
		upgradeWebSocket(() => {
			return {
				onOpen(_event, ws) {
					const isReconnect = gracePeriodTimer !== null;
					cancelGracePeriod();
					...
					if (isReconnect) {
						log(
							`Extension reconnected within grace period — ${connectedTargets.size} targets preserved`,
						);
					} else {
				...
				onClose(_event, ws) {
					...
					// GRACE PERIOD: keep targets + namedPages + playwright clients alive
					// for GRACE_PERIOD_MS. If extension reconnects, state is preserved.
					log(
						`Extension disconnected — ${GRACE_PERIOD_MS / 1000}s grace period before teardown (${connectedTargets.size} targets)`,
					);
